...
class PixelCNN(nn.Module):
    ...
    '''
    def __init__(self, nlayers, in_channels, nfeats, Klevels=2, nclasses=10):
        super(PixelCNN, self).__init__()
        self.layers = nn.ModuleList(
            [GatedConvLayer(in_channels=in_channels, nfeats=nfeats, mask_type='A', kernel_size=7, nclasses=nclasses)])
        for i in range(nlayers):
            gatedconv = GatedConvLayer(in_channels=nfeats, nfeats=nfeats, mask_type='B', kernel_size=5, nclasses=nclasses)
